...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE and start and end:
            ...
            learn(start)
            end_time = time.time()
            total_learn_time = end_time - start_time
            print(f"Total learning time is {total_learn_time}")
            start_time = time.time()
            path = get_shortest_path(start)
            end_time = time.time()
            total_optimal_time = end_time - start_time
            print(
                f"Total time to find shortest path is {total_optimal_time}")
            print(
                f"Total time is {total_optimal_time + total_learn_time}")    
            if(path == None):
